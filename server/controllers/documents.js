  ...
  static retrieveDocuments(req, res) {
    ...
    if (roleId === Constants.ADMIN || roleId === Constants.SUPERADMIN) {
      if(req.query) {
        ...
        .then((documents) => {
          return res.status(200).json(Helper.listContextDetails(documents, limit, offset, 'Documents'));
        });
      }
    } else {
      ...
      .then((documents) => {
        return res.status(200).json(Helper.listContextDetails(documents, limit, offset));
      });
    }
  }
  /**
