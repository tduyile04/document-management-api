...
describe('Users integration tests for the user endpoint', function () {
  ...
  describe('while checking appropriate response during log in', function () {
    ...
    it('should not be able to get a particular user, if user has a regular role', function (done) {
      _chai2.default.request(_app2.default).get('/api/v1/users/10').set('x-access-token', randomToken).end(function (err, res) {
        res.should.have.status(403);
        res.body.message.should.be.eql('You do not have the permission to perform this action');
        done();
      });
    });
    it('should be able to get its own profile, if the user has a regular role', function (done) {
    ...
    it('should not have access to update roles, if user has a regular role', function (done) {
      _chai2.default.request(_app2.default).put('/api/v1/users/1').set('x-access-token', randomToken).send(_Faker2.default.tellingAdmin).end(function (err, res) {
        res.should.have.status(403);
    ...
    it('should not allow empty entries, if user has a regular role', function (done) {
      _chai2.default.request(_app2.default).put('/api/v1/users/1').set('x-access-token', randomToken).send(_Faker2.default.emptyTelling).end(function (err, res) {
        res.should.have.status(400);
  ...
  describe('while checking appropriate response for admin and super admins', function () {
    ...
    it('should be able to update the user detail successfully', function (done) {
      _chai2.default.request(_app2.default).put('/api/v1/users/1').set('x-access-token', adminToken).send(_Faker2.default.adminDataUpdate).end(function (err, res) {
        res.should.have.status(200);
        res.body.should.be.a('object');
